...
def addmusic(request):
    ...
    if form.is_valid():
        ...
        messages.success(request,"Música agregada")
...
def delete(request,id):
    ...
    if music.publisher != request.user:
        messages.error(request, "Usted no está autorizado para acceder a este archivo, o el mismo no existe")
        return redirect("music:dashboard")
    else :
        if music :
            music.delete()
            messages.success(request,"Música eliminada")
